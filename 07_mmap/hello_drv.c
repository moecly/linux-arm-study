...
static ssize_t hello_drv_read(struct file *file, char __user *buf, size_t size,
  ...
  err = copy_to_user(buf, kernel_buf, MIN(1024 * 8, size));
  return MIN(1024 * 8, size);
}
...
int hello_mmap(struct file *file, struct vm_area_struct *vma) {
...
static int __init hello_init(void) {
  ...
  kernel_buf = kmalloc(1024, GFP_KERNEL);
